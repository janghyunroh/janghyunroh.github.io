...
위같은 예시들을 재현해보려고 하면, 예상 외로 정상적인 
이는 chatGPT에서 사용되는 모델이 지속적인 RLHF와 Fine-tuning을 통해 빠르게 대처하기 때문입니다. 

때문에 LLM은 아직 인간을 완전히 대체하지 못합니다. 


LLM을 의사결정 자동화에 사용하는 경우 문제가 발생합니다. 의사결정 과정에서 misalignment 문제가 발생하고나면, 뒤늦게 오류 수정을 위한 Fine-tuning을 해봤자 잘못된 결정으로 인한 피해는 벌어진 뒤니까요. 

때문에 LLM이 생각하는 방식을 어느 정도 제한해야 할 필요가 있습니다. 이를 Grounding이라고 부릅니다. 
...
LLM Knowledge Gap을 해결하기 위한 방법으로 다음 2가지 논문을 리뷰해보고자 합니다. 
