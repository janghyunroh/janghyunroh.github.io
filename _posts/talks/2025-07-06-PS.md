...
때문에 시간 제한이 좀 빡빡하게 걸려 있는 문제들을 풀기 위해선 빠른 입출력 설정이 사실상 필수다.
...
C++에서는 STL에서 구현된 자료구조 클래스를 이용해서 구현한다.
...
q.append(1)    # enqueue (뒤에 추가)
q.popleft()    # dequeue (앞에서 제거 및 반환)
q[0]           # front (앞 원소 확인)
```
{% endcapture %}
...
dq.append(1)      # push_back
dq.popleft()      # pop_front (제거 및 반환)
dq.pop()          # pop_back (제거 및 반환)
```
{% endcapture %}
